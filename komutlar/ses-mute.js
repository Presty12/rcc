 ...
 if(!message.member.roles.cache.has('774944432805117952')) return message.channel.send('**Bu komutu kullanabilmek için \`☭ MUTE AXE\` yetkisine sahip olmasınız.**')
  ...
  if (!mutekisi)
    return message.reply(
      `:warning: Lütfen bir kullanıcı etiketleyiniz! \nDoğru Kullanım; **${prefix}vmute <@kullanıcı> <1sn/1dk/1sa/1g>**`
    );
...
const embed = new Discord.MessageEmbed()
    .setColor('RANDOM')
    .setTitle("Voice Mute İşemi Başarılı")
    .setDescription(`<@${mutekisi.id}> Adlı Kullanıcıya \`${args[1]}\` Süreliğine \`${sebep}\` Sebebiyle Voice Mute Atıldı. Atan Yetkili ${message.author}`)
    .setFooter("NOXUS VOICE MUTE SYSTEM", client.user.avatarURL())
     client.channels.cache.get('775064390712950825').send(embed)
  

  setTimeout(function() {
    mutekisi.roles.remove(muterol.id);
    message.channel.send(`<@${mutekisi.id}> kullanıcısının sesmute süresi sona erdi!`);
  }, ms(mutezaman));
};
